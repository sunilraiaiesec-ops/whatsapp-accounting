...
export default async function CustomerLedgerPage({
  ...
  return (
    <div>
      ...
      {tab === "products" ? <ProductsTab orgId={ctx.orgId} partyId={id} currency={cur} kind="customer" /> : null}

      {tab === "documents" ? <DocumentsTab orgId={ctx.orgId} partyId={id} currency={cur} kind="customer" /> : null}

      {tab === "notes" ? <PartyNotesForm partyId={id} notes={party.notes} /> : null}

      {tab === "ai-memory" ? <AiMemoryTab orgId={ctx.orgId} partyId={id} currency={cur} kind="customer" /> : null}
    </div>
  );
}
...
async function OverviewTab({
  ...
  const [stats, suggestions] = await Promise.all([
    getPartyOverviewStats(orgId, partyId, "customer", currency),
    getPartyEnrichmentSuggestions(orgId, partyId, "customer"),
  ]);
...
async function TransactionsTab({
  ...
  const { rows } = await getPartyLedger(orgId, partyId, "customer", sourceLabels);
...
async function InvoicesTab({ orgId, partyId, currency }: { orgId: string; partyId: string; currency: string }) {
  const invoices = await listPartyInvoices(orgId, partyId, "customer");
...
async function PaymentsTab({ orgId, partyId, currency }: { orgId: string; partyId: string; currency: string }) {
  const receipts = await listPartyPayments(orgId, partyId, "customer");
  const rows: ListRow[] = receipts.map((r) => ({
    ...
    _date: r.date.toISOString().slice(0, 10),
  }));
  return (
    <ListView
      rows={rows}
...
async function ProductsTab({
  ...
  const memory = await getPartyAiMemory(orgId, partyId, kind);
...
async function DocumentsTab({
  ...
  const docs = await listPartyOtherDocuments(orgId, partyId, kind);
...
async function AiMemoryTab({
  ...
  const memory = await getPartyAiMemory(orgId, partyId, kind);
  const verb = kind === "supplier" ? "supplied" : "bought";
    ...
    <div className="space-y-6">
      <div className="card-surface p-5">
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <p className="text-xs font-medium text-slate-500">Most common {kind === "supplier" ? "delivery" : "order"} weekday</p>
