...
import { getSalesReceipt } from "@/lib/documents";
import { formatAmount } from "@/lib/money";
import { TransactionJournal } from "@/components/TransactionJournal";

export default async function SalesReceiptViewPage({
  params,
}: {
  ...
  const data = await getSalesReceipt(ctx.orgId, id);
  if (!data) notFound();
  const { receipt, entry, nav } = data;

  const tax = receipt.lines.reduce((s, l) => s + l.taxAmount, 0n);
  const subtotal = receipt.lines.reduce((s, l) => s + l.lineTotal, 0n);
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-8">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-lg text-slate-700">{receipt.party?.name ?? "Walk-in"}</p>
            <p className="text-sm text-slate-500">Deposited to {receipt.bankAccount.name}</p>
          </div>
          <div className="text-right text-sm">
            ...
            <div className="text-slate-900">{receipt.date.toISOString().slice(0, 10)}</div>
          </div>
        </div>
