...
import { listCreditNotes, docMonthStats } from "@/lib/documents";
...
export default async function CreditNotesPage() {
  ...
  const [notes, stats] = await Promise.all([
    listCreditNotes(ctx.orgId),
    docMonthStats(ctx.orgId, "creditNote"),
  ]);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        title="Credit notes"
        subtitle="Customer returns and refunds — reduce Accounts receivable."
        actionHref="/credit-notes/new"
        actionLabel="New credit note"
      />

      <StatGrid>
        <StatCard icon="doc" tone="emerald" label="Credit notes" value={String(stats.count)} sub="This month" />
      ...
      <div className="mt-8">
        <ListView
          ...
          mobile={{ title: "customer", subtitle: "number", amount: "amount" }}
          columns={[
