...
export default async function SupplierBalancesReportPage() {
  ...
  const { rows, total } = await partyBalanceSummary(ctx.orgId, "supplier");

  return (
    <div className="mx-auto max-w-3xl">
      <ReportBackLink />
      <h1 className="mt-2 text-2xl font-semibold">Supplier Balance Summary</h1>
      <p className="text-sm text-[var(--muted)]">
        Amounts you owe suppliers on accounts payable.
      </p>

      <div className="mt-6 card-surface overflow-hidden">
        {rows.length === 0 ? (
          <p className="p-8 text-center text-sm text-[var(--muted)]">No open supplier balances.</p>
        ) : (
              ...
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-slate-100">
                  <td className="px-4 py-2">
                    <Link href={`/suppliers/${row.id}`} className="font-medium text-[var(--brand)] hover:underline">
                      {row.name}
                    </Link>
                  </td>
