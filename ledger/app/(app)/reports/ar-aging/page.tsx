...
import { receivablesAging, type AgingRow } from "@/lib/reports";
...
export default async function ArAgingReportPage() {
  ...
  const { asOf, rows, totals } = await receivablesAging(ctx.orgId);

  return (
    <div className="mx-auto max-w-5xl">
      <ReportBackLink />
      <h1 className="mt-2 text-2xl font-semibold">Accounts Receivable Aging</h1>
      <p className="text-sm text-[var(--muted)]">
        Unpaid sales invoices as of {asOf.toISOString().slice(0, 10)}.
      </p>

      <div className="mt-6 card-surface overflow-hidden">
        {rows.length === 0 ? (
          <p className="p-8 text-center text-sm text-[var(--muted)]">No unpaid customer invoices.</p>
        ) : (
          <AgingTable rows={rows} totals={totals} cur={cur} partyHref={(id) => `/customers/${id}`} />
        )}
      </div>
    </div>
  );
