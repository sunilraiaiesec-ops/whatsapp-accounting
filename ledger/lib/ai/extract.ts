...
- "sales_invoice": a CREDIT sale to a customer — goods/services sold now, to be PAID LATER (there is no cash/bank movement today). Fields: customer_name, amount (grand total), description, date, due_date (an absolute YYYY-MM-DD date, or null — if the message only gives a RELATIVE term like "net 30", "due in 30 days", "échéance dans 30 jours", leave due_date null and let the app resolve the relative phrase itself; only fill due_date when an explicit calendar date is stated), currency.
- "credit_note": issuing a credit note to a customer — reduces what they owe you (e.g. for a sales return or a price adjustment), with NO cash changing hands. Fields: customer_name, amount, description, date, currency.
- "refund_receipt": an actual CASH refund paid back to a customer (money leaving a bank/cash account now). Fields: customer_name (optional — a refund can be to a walk-in customer with no account), amount, description, date, currency.
